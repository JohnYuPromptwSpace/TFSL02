...
KoreanSMean = {"people":"여러분", "good":"좋은", "sorry":"미안", "hello":"안녕", "great":"잘하다", "nicetosee":"반갑다","meet":"만나다"}
...
modelDuo = load_model('models/model_tfslDuo03.h5')
...
def predict_left(result, handCount):
    ...
    if action_seq_left[-1] == action_seq_left[-2] == action_seq_left[-3]:
        ...
        cv2.putText(img, f'{left_this_act.upper()}', org=(int(res.landmark[0].x * img.shape[1]), int(res.landmark[0].y * img.shape[0] + 20)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255), thickness=2)
...
def predict_right(result, handCount):
    ...
    if action_seq_right[-1] == action_seq_right[-2] == action_seq_right[-3]:
        ...
        cv2.putText(img, f'{right_this_act.upper()}', org=(int(res.landmark[0].x * img.shape[1]), int(res.landmark[0].y * img.shape[0] + 20)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255), thickness=2)
        ...
        if buttonIndex == 1:
            if t.time()-CTIME > 3:
                if final_action != None:
                    if final_action == "nicetosee":
                    ...
                    elif final_action == "hello":
                        if fin_data > 65:
                            if len(transKeys) > 0:
                                if transKeys[-1] == "great":
                                    menuItems[-1] = "안녕하세요"
                                    transKeys[-1] = "hello"
                                    ...
                                    reply = chat.choices[0].message.content
                                    menuItems[-1]=translator.translate(reply, dest=pLang).text
                                else:
                                    message = f"{KoreanSMean[final_action]}을(를) 의문문으로 바꿔줘. 의문문으로 바뀐 문장만 말해. 단어 자체는 바꾸지마"
                                    messages = [{"role": "user", "content": message}]
                                    chat = openai.ChatCompletion.create(
                            ...
                            if len(transKeys) > 0:
                                ...
                                if transKeys[-1] == "great":
                                    menuItems[-1] = "안녕하세요"
                                    transKeys[-1] = "hello"
                                    menuItems[-1]=translator.translate(menuItems[-1], dest=pLang).text
                                else:
                                    transKeys.append(final_action)
                                    menuItems.append(KoreanSMean[final_action])
                                    menuItems[-1]=translator.translate(menuItems[-1], dest=pLang).text
                                    if len(menuItems) > visible_items_count:
                                        starting_index+=1
                            else:
                        ...
                        message = f"{final_action}을(를) 의문문으로 바꿔줘. 의문문으로 바뀐 문장만 말해. 단어 자체는 바꾸지마"
                        messages = [{"role": "user", "content": message}]
                        chat = openai.ChatCompletion.create(
                        model="gpt-4", messages=messages
                        )
